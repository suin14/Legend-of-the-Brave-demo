...
func _input(event: InputEvent) -> void:
	...
	if sd and sd.index == finger_index:
		# 拖动
		var wish_pos := sd.position * get_canvas_transform() - drag_offset  #理想位置
		var movement := (wish_pos - rest_pos).limit_length(DRAG_RADIUS)  #限制拖动范围
		global_position = rest_pos + movement  #最终位置
